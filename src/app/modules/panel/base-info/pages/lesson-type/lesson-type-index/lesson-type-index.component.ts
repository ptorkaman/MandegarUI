...
@Component({
  selector: 'app-lesson-type-index',
  templateUrl: './lesson-type-index.component.html',
  styleUrls: ['./lesson-type-index.component.scss']
})
export class LessonTypeIndexComponent extends BaseComponent implements OnInit {
  ...
  constructor(route: ActivatedRoute,
    ...
    private lessonTypeService: LessonTypeService,    private dataService: DataService,
  ...
  loadLazy(event: LazyLoadEvent | null) {
    this.loading = true;
    this.lessonTypeService.getAll().subscribe(
      (result: any) => {
        if (result.success) {
  ...
  deleteById(id: number) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.lessonTypeService.deletebyId(id).subscribe(
          result => {
            if (result.success) {
