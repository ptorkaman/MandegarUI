...
@Component({
  selector: 'app-study-grade-index',
  templateUrl: './study-grade-index.component.html',
  styleUrls: ['./study-grade-index.component.scss']
})
export class StudyGradeIndexComponent extends BaseComponent implements OnInit {
  ...
  constructor(route: ActivatedRoute,
    ...
    private studyFieldService: StudyGradeService,    private dataService: DataService,
  ...
  private generateTable() {
    this.cols = [
      ...
      { field: 'studyFieldName', header: 'Pages.StudyGrades.StudyFieldId' },
      { field: 'educationBasicCode', header: 'Pages.StudyGrades.EducationBasicCode' }
    ];
  }
  ...
  loadLazy(event: LazyLoadEvent | null) {
    this.loading = true;
    this.studyFieldService.getAll().subscribe(
      (result: any) => {
        if (result.success) {
  ...
  deleteById(id: number) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        ...
        this.studyFieldService.deletebyId(id).subscribe(
          result => {
            if (result.success) {
